...
namespace largefile
{
    ...
    bool MMapFile::sync_file()
    {
        ...
        {
            return msync(data_,size_,MS_ASYNC) == 0; //异步
        }
        return true;
    }
    bool MMapFile :: map_file(const bool write)
    {
        ...
        if(mmap_file_option_.max_mmap_size == 0)
        {
            return false;
        }
        ...
        if(debug) printf("mmap file successed ,fd: %d, mapped size: %d, data : %p\n",fd_,size_,data_);
        return true;
    }
    ...
    bool MMapFile :: ensure_file_size(const int32_t size)
    {
        ...
        if(fstat(fd_,&s) < 0)
        {
            fprintf(stderr,"fstat error desc : %s\n",strerror(errno));
            return false;
        }
        if(s.st_size < size)
        {
    ...
    bool MMapFile :: remap_file()
    {
        ...
        {
            fprintf(stderr,"mmap failed , new size : %d , error desc : %s\n", new_size,strerror(errno));
            return false;
        }
        else
        {
    ...
    void MMapFile::print_option()
    {
        std::cout << mmap_file_option_.max_mmap_size << " " << mmap_file_option_.first_mmap_size << " " << mmap_file_option_.per_mmap_size << std::endl;
    }
}
